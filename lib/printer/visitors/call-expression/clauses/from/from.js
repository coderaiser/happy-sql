import {types} from '@putout/babel';

const {
    isTSAsExpression,
    isCallExpression,
} = types;

const printAsExpression = (path, {write, traverse}) => {
    const expr = path.get('expression');
    
    if (isTableFunc(expr.node))
        printTableFunc(expr, {
            write,
            traverse,
        });
    else
        traverse(expr);
    
    if (isSubqueryAlias(path.node.expression))
        write(' ');
    else
        write(' AS ');
    
    write(path.node.typeAnnotation.literal.value);
};

const isSubqueryAlias = (node) => isCallExpression(node) && [
    'subquery',
    'lateralSubquery',
].includes(node.callee.name);

const CLAUSE_NAMES = [
    'where',
    'groupBy',
    'having',
    'namedWindow',
    'orderBy',
    'limit',
    'limitAll',
    'offset',
    'fetchFirst',
    'forUpdate',
];

const TABLE_ARG_NAMES = [
    ...CLAUSE_NAMES,
    'join',
    'leftJoin',
    'rightJoin',
    'rightOuterJoin',
    'innerJoin',
    'leftOuterJoin',
    'fullOuterJoin',
    'naturalJoin',
    'crossJoin',
    'on',
    'subquery',
    'lateralJoin',
    'lateralSubquery',
    'indexedBy',
    'notIndexed',
];

const isClause = ({node}) => isCallExpression(node) && CLAUSE_NAMES.includes(node.callee.name);

const CAMEL_RE = /([A-Z])/g;
const toSnake = (a) => a.replace(CAMEL_RE, (_, c) => `_${c.toLowerCase()}`);

// single-word ALL-CAPS functions (UNNEST) keep their casing
const ALL_CAPS_RE = /^[A-Z]+$/;
const toFuncName = (name) => ALL_CAPS_RE.test(name) ? name : toSnake(name);

const isTableFunc = (node) => isCallExpression(node) && !TABLE_ARG_NAMES.includes(node.callee.name);

const printTableFunc = (path, {write, traverse}) => {
    const {name} = path.node.callee;
    const args = path.get('arguments');
    
    write(toFuncName(name));
    write('(');
    
    for (const [i, arg] of args.entries()) {
        if (i > 0)
            write(', ');
        
        traverse(arg);
    }
    
    write(')');
};

const printArg = (path, printer) => {
    const {node} = path;
    const {traverse} = printer;
    
    if (isTSAsExpression(node))
        return printAsExpression(path, printer);
    
    if (isTableFunc(node))
        return printTableFunc(path, printer);
    
    traverse(path);
};

export const from = (path, {write, traverse}) => {
    const args = path.get('arguments');
    const tableArgs = args.filter((arg) => !isClause(arg));
    const clauseArgs = args.filter(isClause);
    
    write('FROM ');
    printArg(tableArgs[0], {
        write,
        traverse,
    });
    
    for (let i = 1; i < tableArgs.length; i++)
        printArg(tableArgs[i], {
            write,
            traverse,
        });
    
    for (const clause of clauseArgs) {
        write.breakline();
        traverse(clause);
    }
};
