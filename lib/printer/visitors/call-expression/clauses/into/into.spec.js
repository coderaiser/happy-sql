...
const {file, program, expressionStatement, arrayExpression, callExpression, identifier, stringLiteral} = types;
    ...
    const ast = file(program([
        expressionStatement(arrayExpression([
            callExpression(identifier('into'), [
                identifier('t'),
                identifier('x'),
                callExpression(identifier('values'), [stringLiteral(':x')]),
            ]),
        ])),
    ]));
...
test('happy-sql: printer: clause: into multi col', (t) => {
    const ast = file(program([
        expressionStatement(arrayExpression([
            callExpression(identifier('into'), [
                identifier('t'),
                arrayExpression([identifier('x'), identifier('y')]),
                callExpression(identifier('values'), [stringLiteral(':x'), stringLiteral(':y')]),
            ]),
        ])),
    ]));
