...
const isAccessMethod = (arg) => isCallExpression(arg.node) && arg.node.callee.name === 'accessMethod';
const isCol = (arg) => !isConcurrently(arg) && !isIfNotExists(arg) && !isWhere(arg) && !isAccessMethod(arg);

export const printIndexBody = (path, {write, traverse}) => {
    const [name, table, ...rest] = path.get('arguments');
    ...
    const accessMethod = rest.find(isAccessMethod);
    ...
    if (accessMethod) {
        write(' USING ');
        traverse(accessMethod.get('arguments.0'));
    }
