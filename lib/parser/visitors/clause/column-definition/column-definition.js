...
const isGeneratedExpr = ({expr}) => expr && expr.type === 'paren_expr';

const isStored = (constraint) => Boolean(constraint.storageKw) && constraint.storageKw.name === 'STORED';

const convertGeneratedExpr = (constraint) => callExpression(identifier('generatedAlwaysAs'), [
    convertBinary(constraint.expr.expr),
    booleanLiteral(isStored(constraint)),
]);
...
const getActionName = (kw) => kw.name;
...
const convertReferentialAction = ({eventKw, actionKw}) => {
    ...
    const action = Array.isArray(actionKw) ? actionKw.map(getActionName).join(' ') : actionKw.name;
