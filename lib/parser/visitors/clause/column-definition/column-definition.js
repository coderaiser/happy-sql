...
const convertDefault = ({expr}) => {
    ...
    if (expr.type === 'number_literal')
        return callExpression(identifier('default_'), [numericLiteral(expr.value)]);
    
    if (expr.type === 'string_literal')
        return callExpression(identifier('default_'), [stringLiteral(expr.value)]);
    
    if (expr.type === 'null_literal')
        return callExpression(identifier('default_'), [nullLiteral()]);
    
    if (expr.type === 'boolean_literal')
        return callExpression(identifier('default_'), [booleanLiteral(expr.value)]);
