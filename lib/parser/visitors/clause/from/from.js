...
export const convertFrom = ({expr, whereClause}) => {
    ...
    if (whereClause)
        fromArgs.push(callExpression(identifier('where'), [convertCondition(whereClause.expr)]));
    
    return callExpression(identifier('from'), fromArgs);
};
