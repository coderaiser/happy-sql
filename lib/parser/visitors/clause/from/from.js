...
const convertCondition = (node) => {
    ...
    if (isExists(node))
        return callExpression(identifier('exists'), [convertClause(node.expr.expr)]);
...
const convertAlias = (node) => {
    ...
    if (isSubquery(node.expr)) {
        ...
        return tsAsExpression(
            callExpression(identifier('subquery'), [convertClause(inner)]),
...
const convertJoinType = (operator) => {
    ...
    const camel = parts[0].toLowerCase() + parts.slice(1).map((s) => s[0].toUpperCase() + s.slice(1).toLowerCase()).join('');
