...
const isJoinUsing = ({type}) => type === 'join_using_specification';

const convertSpecification = (spec) => {
    if (isJoinUsing(spec))
        return callExpression(identifier('using'), spec.expr.expr.items.map(({name}) => identifier(name)));
    
    return convertOn(spec);
};
...
const convertAlias = (node) => {
    ...
    if (node.expr.type === 'func_call')
        return tsAsExpression(convertTableFunc(node.expr), tsLiteralType(stringLiteral(node.alias.name)));
...
const capitalizeWord = (s) => s[0].toUpperCase() + s.slice(1).toLowerCase();
...
const convertJoinType = (operator) => {
    ...
    for (const part of parts.slice(1))
        tail.push(capitalizeWord(part));
    
    const camel = parts[0].toLowerCase() + tail.join('');
    ...
    if (node.type === 'join_expr') {
        ...
        if (isCrossJoin(node))
            ...
            joins.push(callExpression(identifier(fnName), [
                convertAlias(node.right),
                convertSpecification(node.specification),
            ]));
...
export const convertFrom = ({expr, whereClause, groupByClause, havingClause, orderByClause, windowClause, limitClause, offsetClause, fetchClause, forClause}) => {
    const fromArgs = [];
    ...
    if (offsetClause)
        fromArgs.push(convertOffset(offsetClause));
    
    if (fetchClause)
        fromArgs.push(convertFetch(fetchClause));
    
    if (forClause)
        fromArgs.push(convertForUpdate());
