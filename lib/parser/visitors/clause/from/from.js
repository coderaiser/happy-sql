...
const convertAlias = (node) => {
    if (node.type === 'alias')
        return callExpression(identifier('as'), [identifier(node.expr.name), identifier(node.alias.name)]);
...
export const convertFrom = (node) => {
    ...
    if (expr.type === 'alias')
        return callExpression(identifier('from'), [convertAlias(expr)]);
    
    return callExpression(identifier('from'), [identifier(expr.name)]);
};
