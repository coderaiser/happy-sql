...
const isDesc = (item) => item.direction && item.direction.type === 'sort_direction_desc';
...
const convertColumns = (columns) => {
    ...
    for (const item of items) {
        if (isDesc(item))
            cols.push(callExpression(identifier('desc'), [
                identifier(item.expr.name),
            ]));
        else
...
export const convertCreateIndex = (stmt) => {
    ...
    if (stmt.using)
        args.push(callExpression(identifier('accessMethod'), [
            identifier(stmt.using.method.name),
        ]));
    
    args.push(...convertColumns(columns));
    ...
    return callExpression(identifier(isUnique ? 'createUniqueIndex' : 'createIndex'), args);
};
