...
const clauseVisitors = {
    ...
    reindex_stmt: (stmt) => callExpression(identifier('reindex'),
        stmt.name ? [identifier(stmt.name.name)] : []),
    analyze_stmt: (stmt) => callExpression(identifier('analyze'), (stmt.tables?.items ?? []).map((t) => identifier(t.name))),
