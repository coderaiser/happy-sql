...
import {convertGrant} from '#parser/clause/grant';
...
const isMemberExpr = ({type}) => type === 'member_expr';

const convertTableName = (table) => {
    if (isMemberExpr(table))
        return convertMemberExpr(table);
...
const getAnalyzeTables = ({tables}) => tables.items || tables;
...
const clauseVisitors = {
    ...
    truncate_stmt: ({tables}) => {
        const args = [];
        
        for (const table of tables.items)
            args.push(convertTableName(table));
        
        return callExpression(identifier('truncate'), args);
    },
    ...
    grant_privilege_stmt: convertGrant,
    reindex_stmt: (stmt) => callExpression(identifier('reindex'), stmt.name ? [
        identifier(stmt.name.name),
    ] : []),
    analyze_stmt: (stmt) => {
        const args = [];
        
        for (const table of getAnalyzeTables(stmt))
            args.push(convertTableName(table));
        
        return callExpression(identifier('analyze'), args);
    },
    release_savepoint_stmt: ({savepoint}) => callExpression(identifier('releaseSavepoint'), [
        identifier(savepoint.name),
    ]),
    do_stmt: (stmt) => callExpression(identifier('doBlock'), [
        stringLiteral(stmt.body.value),
    ]),
};
