...
const convertFilter = (column, fn) => {
    ...
    return callExpression(identifier('filter'), [
        fn,
        condition,
    ]);
};
...
const convertColumn = (column) => {
    if (isCount(column))
        return convertFilter(column, convertCount(column));
