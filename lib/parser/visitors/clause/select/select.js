...
const convertFrameBound = (bound) => {
    ...
    const name = bound.type === 'frame_bound_preceding' ? 'preceding' : 'following';
        ...
        const isUnbounded = bound.expr && bound.expr.type === 'frame_unbounded';
    const args = !isUnbounded && bound.expr ? [
...
const convertFuncCall = (column) => {
    ...
    for (const item of items)
        args.push(convertOperand(item));
            ...
            if (column.over) {
        const funcName = LOWERCASE_FUNCS.has(lower) ? lower : name;
        return convertOver(column, callExpression(identifier(funcName), args));
    }
...
const isSelectDistinct = ({type}) => type === 'select_distinct';
const isSelectDistinctOn = ({type}) => type === 'select_distinct_on';

const isDistinct = (selectClause) => selectClause.modifiers.some(isSelectDistinct);

const convertDistinctOn = ({modifiers}) => {
    if (!modifiers.some(isSelectDistinctOn))
        return null;
    
    const modifier = modifiers.find(isSelectDistinctOn);
    const onCols = [];
...
export const convertSelect = ({clauses}) => {
    ...
    const offsetClause = findClause(clauses, 'offset_clause');
    const fetchClause = findClause(clauses, 'fetch_clause');
    const forClause = findClause(clauses, 'for_clause');
    ...
    if (!fromClause) {
        if (limitClause)
            args.push(convertLimit(limitClause));
        
        if (offsetClause)
            args.push(convertOffset(offsetClause));
        
        if (fetchClause)
            args.push(convertFetch(fetchClause));
        
        if (forClause)
            args.push(convertForUpdate());
    }
