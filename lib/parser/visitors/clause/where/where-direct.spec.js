...
test('happy-sql: parser: clause: where: NOT EXISTS becomes notExists', (t) => {
    const source = 'SELECT * FROM t WHERE NOT EXISTS (SELECT 1 FROM u)';
    const cst = parseCst(source, {
        dialect: 'postgresql',
        paramTypes: [':name'],
    });
    
    const [whereClause] = cst.statements[0].clauses.filter(({type}) => type === 'where_clause');
    const ast = convertWhere(whereClause);
    const result = toString(ast);
    const expected = 'where(notExists(select(1, from(u))));';
    
    t.equal(result, expected);
    t.end();
});

test('happy-sql: parser: clause: where: paren expr unwraps', (t) => {
    const source = 'SELECT * FROM t WHERE (x = 1 AND y = 2)';
    const cst = parseCst(source, {
        dialect: 'postgresql',
        paramTypes: [':name'],
    });
